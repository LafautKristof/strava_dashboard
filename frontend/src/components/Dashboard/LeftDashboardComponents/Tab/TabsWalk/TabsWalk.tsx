...
const TabsWalk = ({ data }: { data: DataActivity }) => {
    const { totalDistanceKm, formattedTime, totalElevationGain, daysData } =
        getWeeklyStatsByType(data.activities, "Walk");

    const yearly = getCurrentYearlyStatsByType(data.activities, "Walk");
    const {
